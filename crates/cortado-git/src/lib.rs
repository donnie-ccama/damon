//! Git operations for cortado agents. Shells out to the git CLI.
...
const BLOCK_BEGIN: &str = "# cortado begin";
const BLOCK_END: &str = "# cortado end";
/// Every bridge filename cortado has ever excluded — used to absorb legacy
/// unmarked lines from pre-block installs into the block.
const KNOWN_PATTERNS: [&str; 3] = ["CLAUDE.md", "AGENTS.md", ".claude/settings.json"];

/// The bridge filenames cortado-git recognizes for legacy-line migration.
/// Kept in sync with `cortado_core::bridge::write_bridges` by a cross-crate
/// test in the `cortado` crate (`tests/bridge_exclude_sync.rs`).
...
/// exclude file, so two concurrent `cortado open`s cannot lose an update. The
...
fn with_exclude_lock<T>(
    ...
    let lock_path = info.join(".cortado-exclude.lock");
...
/// never touched, except legacy cortado patterns, which migrate into the block.
...
/// Remove cortado's block (and any legacy cortado lines). `repo` is any path
...
fn upsert_block(existing: &str, entries: &[&str]) -> String {
    ...
    // Outside the markers only cortado's own patterns are ever dropped
...
fn write_file(path: &Path, content: &str) -> Result<(), GitError> {
    ...
    let tmp = path.with_extension("cortado-tmp");
