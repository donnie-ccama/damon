...
fn init_new_creates_repo_on_branch() {
    ...
    cortado_git::init_new(&wt, "agent/scout").unwrap();
...
fn clone_checks_out_agent_branch() {
    ...
    let wt = tmp.path().join("worktree");
    cortado_git::clone_repo(origin.to_str().unwrap(), &wt, "agent/scout").unwrap();
    assert_eq!(git(&wt, &["branch", "--show-current"]), "agent/scout");
...
fn worktree_add_and_remove() {
    ...
    let origin = seed_origin(tmp.path());
    let wt = tmp.path().join("worktree");
    cortado_git::worktree_add(&origin, &wt, "agent/scout").unwrap();
    assert_eq!(git(&wt, &["branch", "--show-current"]), "agent/scout");
    cortado_git::worktree_remove(&origin, &wt).unwrap();
    assert!(!wt.exists());
}
...
fn worktree_add_attaches_existing_branch_and_reports_real_errors() {
    ...
    cortado_git::worktree_add(&origin, &wt1, "agent/scout").unwrap();
    cortado_git::worktree_remove(&origin, &wt1).unwrap();
    // branch agent/scout still exists; re-add must attach, not fail on -b
    let wt2 = tmp.path().join("wt2");
    cortado_git::worktree_add(&origin, &wt2, "agent/scout").unwrap();
    ...
    let err = cortado_git::worktree_add(&origin, &bad, "agent/other").unwrap_err();
...
fn exclude_writes_marked_block_idempotently() {
    ...
    cortado_git::init_new(&wt, "main").unwrap();
    cortado_git::exclude(&wt, &["CLAUDE.md", ".claude/settings.json"]).unwrap();
    let first = std::fs::read_to_string(exclude_file(&wt)).unwrap();
    assert!(first.contains("# cortado begin\nCLAUDE.md\n.claude/settings.json\n# cortado end\n"));
    cortado_git::exclude(&wt, &["CLAUDE.md", ".claude/settings.json"]).unwrap();
    let second = std::fs::read_to_string(exclude_file(&wt)).unwrap();
    assert_eq!(first, second); // byte-identical on repeat
    assert_eq!(first.matches("# cortado begin").count(), 1);
}
...
fn exclude_merges_entries_across_calls() {
    ...
    let wt = tmp.path().join("repo");
    cortado_git::init_new(&wt, "main").unwrap();
    cortado_git::exclude(&wt, &["CLAUDE.md"]).unwrap();
    cortado_git::exclude(&wt, &["AGENTS.md"]).unwrap();
    let text = std::fs::read_to_string(exclude_file(&wt)).unwrap();
    assert!(text.contains("# cortado begin\nCLAUDE.md\nAGENTS.md\n# cortado end\n"));
    assert_eq!(text.matches("# cortado begin").count(), 1);
}

#[test]
fn exclude_preserves_user_lines_and_migrates_legacy_cortado_lines() {
    isolate_git();
    let tmp = tempfile::tempdir().unwrap();
    let wt = tmp.path().join("repo");
    cortado_git::init_new(&wt, "main").unwrap();
    ...
    // A user pattern plus a pre-M4 unmarked cortado line.
...
fn exclude_remove_deletes_only_the_block() {
    ...
    std::fs::create_dir_all(file.parent().unwrap()).unwrap();
    std::fs::write(&file, "user-pattern\n").unwrap();
    cortado_git::exclude(&wt, &["CLAUDE.md", "AGENTS.md"]).unwrap();
    cortado_git::exclude_remove(&wt).unwrap();
    let text = std::fs::read_to_string(&file).unwrap();
...
fn exclude_remove_without_exclude_file_is_ok() {
    ...
    let wt = tmp.path().join("repo");
    cortado_git::init_new(&wt, "main").unwrap();
    cortado_git::exclude_remove(&wt).unwrap(); // file never written — no error
}
...
fn exclude_remove_propagates_non_missing_read_errors() {
    ...
    assert!(cortado_git::exclude_remove(&wt).is_err());
}
...
fn exclude_propagates_non_missing_read_errors_instead_of_clobbering() {
    ...
    std::fs::write(&file, [0xFF, 0xFE, 0xFD]).unwrap();
    assert!(cortado_git::exclude(&wt, &["CLAUDE.md"]).is_err());
...
fn common_dir_matches_for_source_repo_and_its_worktree() {
    ...
    cortado_git::worktree_add(&origin, &wt, "agent/scout").unwrap();
    let a = cortado_git::common_dir(&origin).unwrap();
    let b = cortado_git::common_dir(&wt).unwrap();
