...
pub fn set(provider: &str) -> anyhow::Result<()> {
    if std::env::var("CORTADO_NO_KEYRING").is_ok_and(|v| !v.is_empty()) {
        anyhow::bail!("OS keyring disabled (CORTADO_NO_KEYRING is set)");
    }
    ...
    keyring::Entry::new("cortado", provider)?.set_password(key)?;
    println!("stored key for {provider} (OS keyring, service \"cortado\")");
    Ok(())
}
...
pub fn rm(provider: &str) -> anyhow::Result<()> {
    ...
    let entry = keyring::Entry::new("cortado", provider)?;
