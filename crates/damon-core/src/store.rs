...
impl Store {
    ...
    pub fn create_team(&self, name: &str) -> Result<Slug, CoreError> {
        ...
        std::fs::create_dir_all(dir.join("agents")).map_err(|e| CoreError::Io {
            path: dir.clone(),
            source: e,
        })?;
        let team = TeamFile {
        ...
        let text = toml::to_string_pretty(&team).map_err(|e| CoreError::Invalid(e.to_string()))?;
        std::fs::write(dir.join("team.toml"), text).map_err(|e| CoreError::Io {
            path: dir.join("team.toml"),
            source: e,
        })?;
        Ok(slug)
    }
    ...
    pub fn resolve(&self, reference: &str) -> Result<AgentEntry, CoreError> {
        ...
        match matches.len() {
            ...
            _ => {
                let teams: Vec<String> = matches
                    .iter()
                    .map(|a| format!("{}/{}", a.team, a.slug))
                    .collect();
...
mod tests {
    ...
    fn fixture() -> (tempfile::TempDir, Store) {
        ...
        let dir = store.agent_dir(
            &Slug::parse("newsletter").unwrap(),
            &Slug::parse("scout").unwrap(),
        );
    ...
    fn create_team_writes_team_toml_and_rejects_dupes() {
        ...
        assert!(teams
            .iter()
            .any(|t| t.slug.as_str() == "newsletter" && t.team.is_ok()));
        assert!(matches!(
            store.create_team("Newsletter"),
            Err(CoreError::Exists(_))
        ));
    }
    ...
    fn discovers_agents_and_reports_invalid_toml() {
        let (_tmp, store) = fixture();
        let bad = store.agent_dir(
            &Slug::parse("church").unwrap(),
            &Slug::parse("broken").unwrap(),
        );
        ...
        assert!(all
            .iter()
            .any(|a| a.slug.as_str() == "broken" && a.agent.is_err()));
    }
    ...
    fn resolve_by_path_and_bare_slug() {
        let (_tmp, store) = fixture();
        assert_eq!(
            store.resolve("newsletter/scout").unwrap().slug.as_str(),
            "scout"
        );
    ...
    fn agent_dir_without_toml_reports_invalid() {
        let (_tmp, store) = fixture();
        let empty = store.agent_dir(
            &Slug::parse("church").unwrap(),
            &Slug::parse("hollow").unwrap(),
        );
